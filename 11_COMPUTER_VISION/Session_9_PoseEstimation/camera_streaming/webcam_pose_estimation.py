...
def main():
    ...
    try:
        ...
        camera = CameraClient(host='100.89.173.92')
    ...
    if isinstance(camera, cv2.VideoCapture):
        ...
        # For CameraClient, we'll check the connection by trying to get a frame
        try:
            test_frame = camera.get_frame()
            if test_frame is None:
                raise IOError("Cannot get frame from remote camera")
        except Exception as e:
            raise IOError(f"Failed to connect to remote camera: {e}")
    ...
    while True:
        ...
        if isinstance(camera, cv2.VideoCapture):
            ...
            try:
                ...
                if frame is None:
                    print("Lost connection to remote camera")
                    break
            except Exception as e:
