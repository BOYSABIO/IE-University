...
# Draw the bar chart of the percentage of models of each make
"""percentage = df.groupby("Make")["Model"].value_counts(normalize = True)

fig, ax = plt.subplots()
df1 = df.groupby("Make")["Model"].value_counts(normalize = True)*100
df1.plot(kind = "bar", stacked = True)
plt.show()"""

"""#print(df["Make"].value_counts())

percentage_models_by_make = df.groupby('Make')['Model'].value_counts(normalize=True).mul(100).reset_index(name='Percentage')

# Pivot the DataFrame to have 'Make' as index and 'Model' as columns
pivot_df = percentage_models_by_make.pivot(index='Make', columns='Model', values='Percentage')

# Plot a stacked bar chart
ax = pivot_df.plot(kind='bar', stacked=True, figsize=(10, 6))
ax.set_ylabel('Percentage of Models')
ax.set_xlabel('Make')
ax.set_title('Percentage of Models for Each Make')
plt.legend(title='Model', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.show()"""


"""models_total = df["Make"].value_counts(ascending = False)
make_model = df.groupby("Make")["Model"].value_counts()
print(make_model)
print(make_model.head(10).sum())
